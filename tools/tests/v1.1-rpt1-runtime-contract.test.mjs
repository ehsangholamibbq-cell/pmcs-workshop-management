...
test("RPT1-F03 connects its checkpointed semantic runtime and renderers without production enablement", () => {
  ...
  assert.match(roadmap, /نسخه سند: `1\.50\.0`/u);
...
test("RPT1-F03 Catalog API and Worker record the S07-MS09 connected safe checkpoint", () => {
  ...
  assert.match(canonical, /Safe Resume Point قطعی فعلی آن `PMCS-V1\.1-RPT1-S07-MS16-C1`/u);
  assert.match(canonical, /PMCS-RPT1-F04-SEMANTIC-001 v1\.0\.0[\s\S]*Run 158/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 156/u);
  assert.match(baseline, /Slice 07 Micro-Step 09[\s\S]*Run 156/u);
  assert.match(matrix, /## ۳۱\.[\s\S]*Run 156/u);
  assert.match(matrix, /نسخه: `1\.33\.0`/u);
...
test("RPT1-F04 Runtime Core records the S07-MS11 safe checkpoint without opening renderer or wiring", () => {
  ...
  assert.match(canonical, /Safe Resume Point قطعی فعلی آن `PMCS-V1\.1-RPT1-S07-MS16-C1`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 163/u);
...
test("RPT1-F04 Renderer and Golden record the S07-MS12 safe checkpoint without opening wiring", () => {
  ...
  assert.match(canonical, /Safe Resume Point قطعی فعلی آن `PMCS-V1\.1-RPT1-S07-MS16-C1`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 167/u);
...
test("RPT1-F04 Catalog API and Worker record the S07-MS13 connected safe checkpoint", () => {
  ...
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 169/u);
  ...
  assert.match(rootReadme, /PMCS-V1\.1-RPT1-S07-MS16-C1[\s\S]*F05 اکنون[\s\S]*`Renderer\/Golden Safe Checkpoint \/ Catalog\/API\/Worker Not Implemented`/u);
...
test("RPT1-F05 keeps the certified financial position contract aligned with its Runtime and Renderer", () => {
  ...
  assert.match(contract, /Parent checkpoint: `PMCS-V1\.1-RPT1-S07-MS15-C1`/u);
  assert.match(contract, /پارامتر معنایی Client دقیقاً یک object خالی `\{\}`/u);
  ...
  assert.match(architecture, /PMCS-RPT1-F05-SEMANTIC-001 v1\.2\.1/u);
  ...
  assert.match(canonical, /F05[\s\S]*`Renderer\/Golden Safe Checkpoint \/ Catalog\/API\/Worker Not Implemented`[\s\S]*F06 تا F10[\s\S]*`Required \/ Not Implemented`/u);
  assert.match(rootReadme, /Runtime\/Renderer\/Golden checkpointed F05 برای وضعیت مالی/u);
...
test("RPT1-F05 Renderer and Golden record the S07-MS16 safe checkpoint without opening wiring", () => {
  ...
  assert.match(canonical, /گام بعدی فقط wiring متصل خانواده `RPT1-F05`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 178/u);
  ...
  assert.match(matrix, /نسخه: `1\.33\.0`/u);
  assert.match(rootReadme, /PMCS-V1\.1-RPT1-S07-MS16-C1[\s\S]*Renderer\/Golden قطعی `RPT1-F05`/u);
...
test("RPT1-F05 Runtime Core records the S07-MS15 safe checkpoint without opening renderer or wiring", () => {
  ...
  assert.match(canonical, /گام بعدی فقط wiring متصل خانواده `RPT1-F05`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 175/u);
  assert.match(contract, /نسخه: `1\.2\.1`[\s\S]*Renderer\/Golden Safe Checkpoint/u);
...
test("RPT1-F05 semantic contract preserves the historical S07-MS14 checkpoint", () => {
  ...
  assert.match(canonical, /گام بعدی فقط wiring متصل خانواده `RPT1-F05`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 171/u);
  assert.match(contract, /نسخه: `1\.2\.1`[\s\S]*Renderer\/Golden Safe Checkpoint/u);
...
test("RPT1-F04 semantic contract records the S07-MS10 safe checkpoint without claiming Runtime", () => {
  ...
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 158/u);
...
test("RPT1-F03 semantic contract records the S07-MS06 safe checkpoint without claiming Runtime", () => {
  ...
  assert.match(checkpoint, /Safe Resume Point اکنون `PMCS-V1\.1-RPT1-S07-MS06-C1`/u);
  assert.match(checkpoint, /Micro-Step بعدی فقط Runtime identity/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 146/u);
...
test("RPT1-F03 Runtime Core records the S07-MS07 safe checkpoint without opening renderer or wiring", () => {
  ...
  assert.match(canonical, /Safe Resume Point قطعی فعلی آن `PMCS-V1\.1-RPT1-S07-MS16-C1`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 148/u);
...
test("RPT1-F03 Renderer and Golden record the S07-MS08 safe checkpoint without opening wiring", () => {
  ...
  assert.match(canonical, /Safe Resume Point قطعی فعلی آن `PMCS-V1\.1-RPT1-S07-MS16-C1`/u);
  assert.match(architecture, /نسخه: `1\.30\.0`[\s\S]*Run 154/u);
...
test("RPT1-F02 renderer contract stays deterministic while wiring remains production-disabled", () => {
  ...
  assert.match(canonical, /Catalog\/API\/Worker و qualification متصل F01 تا F04 و Runtime\/Renderer\/Golden F05 بسته/u);
});
